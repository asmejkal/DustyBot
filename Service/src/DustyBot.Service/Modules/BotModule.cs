...
namespace DustyBot.Service.Modules
{
    ...
    internal sealed class BotModule : IDisposable
    {
        ...
        public async Task About(ICommand command)
        {
            ...
            var embed = new EmbedBuilder()
                .WithTitle($"{_client.CurrentUser.Username} (DustyBot v{typeof(Program).GetTypeInfo().Assembly.GetCustomAttribute<AssemblyFileVersionAttribute>().Version})")
                .AddField("Author", "Yeba#3517", true)
                .AddField("Owner", _botOptions.Value.OwnerID, true)
