...
namespace DustyBot.DaumCafe
{
    ...
    /// </summary>
    public sealed class DaumCafeSession : IDisposable
    {
        public static readonly DaumCafeSession Anonymous = new DaumCafeSession();
        
        private static readonly Regex _metaPropertyRegex = new Regex(@"<meta\s+property=""(.+)"".+content=""(.+)"".*>", RegexOptions.Compiled);
        private static readonly Regex _boardLinkRegex = new Regex(@".*cafe.daum.net\/(.+)\/(\w+).*", RegexOptions.Compiled);
        private static readonly Regex _bbsBoardLinkRegex = new Regex(@".*cafe.daum.net\/.+\/bbs_list.+fldid=(\w+).*", RegexOptions.Compiled);
        private static readonly Regex _groupCodeRegex = new Regex(@"GRPCODE\s*:\s*""(.+)""", RegexOptions.Compiled);
        ...
        public async Task<DaumCafePage> GetPage(string cafeId, string boardId, int postId, CancellationToken ct)
        {
            string content;
            var mobileUri = new Uri($"http://m.cafe.daum.net/{cafeId}/{boardId}/{postId}");
            var desktopUri = new Uri($"http://cafe.daum.net/{cafeId}/{boardId}/{postId}");

            var response = await _client.GetAsync(mobileUri, ct);
            if (response.StatusCode == (HttpStatusCode)308)
            {
                ...
                if (location == null)
                    throw new HttpRequestException("A 308 redirect is missing the location header");

                var absoluteLocation = location.IsAbsoluteUri ? location : new Uri(new Uri(mobileUri.GetComponents(UriComponents.Scheme | UriComponents.StrongAuthority, UriFormat.Unescaped)), location);
            ...
            if (!string.IsNullOrEmpty(url) && url.Contains("comments"))
            {
                ...
                return new DaumCafePage(
                    ...
                    boardId,
                    postId,
                    DaumCafePageType.Comment,
                    DaumCafePageBody.CreateFromComment(content),
                    mobileUri,
                    desktopUri);
            }
            else
            {
                ...
                var type = properties.FirstOrDefault(x => x.Item1 == "og:type")?.Item2;
                var title = properties.FirstOrDefault(x => x.Item1 == "og:title")?.Item2;
                var description = properties.FirstOrDefault(x => x.Item1 == "og:description")?.Item2;
                return new DaumCafePage(
                    cafeId,
                    boardId,
                    postId,
                    type == "article" ? DaumCafePageType.Article : null,
                    DaumCafePageBody.Create(content),
                    mobileUri,
                    desktopUri,
        ...
        {
            if (_credential == null)
                throw new InvalidOperationException("No credential specified");
